...
```text
Assessment -> Draft Quote -> quote version/status handling -> sent / accepted / rejected quote path
```
...
```text
Lead -> may become first Client + Property + Assessment
```

Relationship rules:

- Lead is the raw enquiry / early triage object.
- Client is the top-level relationship, payer, and main-contact object.
...
- Do not introduce `work_label` as the normal workflow identity in lists or headers.
- Use structured display identity: customer/client, property/address, service, purpose, status, quote.
...
- Assessments now use workspace-first layout.
- Assessments have Active Assessments and Assessment History containers.
- Assessment records can be marked not proceeding with a reason and optional note.
- Closed Assessments move into Assessment History.
- Client & Home now uses the same active/history workspace architecture.
- Assessments and Client & Home use a collapsed drawer rail by default.
- Current accepted assessment/quote flow can still link into Client & Home as a temporary bridge.
...
- admin can reject / mark not suitable without forcing Assessment creation
...
- Assessments and Client & Home use workspace-first active/history architecture.
- Assessments support not proceeding close-out with reason and optional note.
...
- add notes/tasks
- decide whether it should move into an Assessment
...
Client is the top-level relationship object. Property/Home sits below Client.
...
- Client & Home uses the same workspace-first architecture as Assessments.
...
- Client & Home is the operational customer/property hub after acceptance/conversion.
- Existing-client extra work should create a new Assessment under the same Client, not a duplicate Client.
...
Assessment is not the commercial quote itself. It is the internal working record that gathers:
...
Current bridge behaviour: an accepted assessment/quote can still link into Client & Home. This is temporary. The long-term intended trigger is accepted Quote creating the next delivery layer rather than Assessment directly becoming the delivery object.

Clear distinction:

- **Lead** = early enquiry / triage only.
- **Assessment** = internal working record for evaluation, scope, notes and Quote Assist.
...
- **Job / Work Order** = future delivery container created after accepted Quote.
- **Visit / Appointment** = future scheduled occurrence under a Job or recurring plan.
- **Invoice** = future payment request.
...
Current Assessment behaviour:

- Active Assessment records are live assessment/quote-prep work.
- Assessment History holds converted, closed, rejected, not proceeding and similar final records.
...
- Prospect grouping: Lead/Assessment prospect -> quotes -> quote versions.
...
- Assessment is the internal working record.
...
```text
Lead -> Assessment -> Quote Assist / Draft Quote -> Assessment can be closed as not proceeding -> temporary accepted assessment/quote -> Client & Home bridge
```
...
-> Quote sent
-> Quote accepted / rejected / expired
-> accepted Quote creates or links Client & Home
-> Property / Home context
-> Jobs / Visits
-> Reports / Follow-ups
-> Billable Events
...
- Quote Assist foundation sufficient for current Assessment use
- Lead lost/reject/not suitable handling
- Assessment / C&H workspace architecture
- Assessment close/not proceeding workflow

Next selected phase:

### Accounting Quote Workflow MVP

- quote records linked to Assessment and Lead where applicable
...
### Phase 4 - Assessment / Client Workspace Foundation

- Assessment workspace-first layout
- Assessment Active / History containers
- collapsed drawer rail
- Client & Home workspace-first layout
- active / history structure for Client & Home
- Assessment not proceeding close-out path
...
- Quote Assist inside Assessments
- Assessment active/history workflow
- Client & Home workspace foundation
- draft quote foundation linked to Assessments
- current temporary accepted assessment/quote -> Client & Home bridge
